...
class PlaybackPlanCatalogueClient(Protocol):
...
async def create_launch_token(
    catalogue: PlaybackPlanCatalogueClient, *, user: str, context: PlaybackPlanContext
) -> str:
    """Resolve a user reference and create one opaque Katalog launch token."""

    user_id = await resolve_user_id(catalogue, user)
    try:
        request: PlaybackPlanRequest = PlaybackPlanRequest(user_id=user_id, context=context)
        launch: PlaybackPlanLaunch = await catalogue.create_playback_plan(request)
    except Exception as error:
        raise KestrelPlaybackError("Katalog could not create a playback plan.") from error
    return launch.launch_token


async def resolve_user_id(catalogue: PlaybackPlanCatalogueClient, user: str) -> int:
    """Resolve a positive numeric ID or exactly one configured username."""

    normalised: str = user.strip()
    if not normalised:
        raise KestrelPlaybackError("A playback user is required.")
    if normalised.isdecimal():
        user_id: int = int(normalised)
        if user_id > 0:
            return user_id
        raise KestrelPlaybackError("A playback user ID must be positive.")
    try:
        users: tuple[UserSummary, ...] = await catalogue.list_users()
    except Exception as error:
        raise KestrelPlaybackError("Katalog users could not be resolved.") from error
    matching_users: tuple[UserSummary, ...] = tuple(
        candidate for candidate in users if candidate.username == normalised
    )
