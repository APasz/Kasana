  ...
  class KanvasAdministration extends HTMLElement {
    ...
    rootRow(root) {
      ...
      return `<article class="k-root-row" data-root-id="${Number(root.id)}">
        ...
        <div><span class="k-admin-state${stateClass}">${state}</span><small>Last scan ${lastScanLabel(root.lastScanCompletedAt)}</small><details class="k-admin-row__details"><summary>Settings</summary><div><code>${escapeHtml(root.path || 'Path unavailable')}</code><span>Required mount ${escapeHtml(root.requiredMountPath || 'none')}</span><span>${tags}</span><span>Audio ${escapeHtml(root.preferredAudioLanguage || 'stream default')} · Subtitles ${escapeHtml(root.preferredSubtitleLanguage || 'stream default')}</span></div></details></div>
    ...
    rootDialog(root) {
      ...
      dialog.innerHTML = `<form method="dialog" class="k-picker k-admin-root-form" data-admin-root-form><div class="k-picker__header"><strong>${root ? 'Edit library root' : 'Add library root'}</strong></div><label class="k-control-shell k-input-shell"><input class="k-input" name="displayName" value="${escapeHtml(root?.displayName || '')}" placeholder="Name" aria-label="Root name"></label><div class="k-admin-root-path-row"><label class="k-control-shell k-input-shell"><input class="k-input" name="path" value="${escapeHtml(root?.path || '')}" placeholder="Path" aria-label="Root path" data-admin-root-path required></label><button type="button" class="k-button" data-admin-root-browse>Browse</button></div><div class="k-directory-picker" data-admin-directory-picker hidden></div><label class="k-control-shell k-input-shell"><input class="k-input" name="requiredMountPath" value="${escapeHtml(root?.requiredMountPath || '')}" placeholder="Required mount point (optional)" aria-label="Required mount point"></label><label class="k-control-shell k-select-wrap"><select class="k-select" name="kind" aria-label="Root kind"><option value="movie"${root?.kind === 'movie' ? ' selected' : ''}>Movie</option><option value="series"${root?.kind === 'series' ? ' selected' : ''}>Series</option></select></label><label class="k-control-shell k-input-shell"><input class="k-input" name="tags" value="${escapeHtml((root?.tags || []).join(', '))}" placeholder="Tags" aria-label="Root tags"></label><label class="k-control-shell k-input-shell"><input class="k-input" name="preferredAudioLanguage" value="${escapeHtml(root?.preferredAudioLanguage || '')}" placeholder="Preferred audio language (for example, en)" aria-label="Preferred audio language"></label><label class="k-control-shell k-input-shell"><input class="k-input" name="preferredSubtitleLanguage" value="${escapeHtml(root?.preferredSubtitleLanguage || '')}" placeholder="Preferred subtitle language (for example, en)" aria-label="Preferred subtitle language"></label><label class="k-control-shell k-check"><input type="checkbox" name="enabled"${root?.enabled !== false ? ' checked' : ''}> Enabled</label><div class="k-action-row"><button type="submit" class="k-button k-button--primary">Save root</button><button type="button" class="k-button" data-admin-root-close>Cancel</button></div></form>`;
    ...
    saveRoot(root, form) {
      return this.operation(root ? 'root-update' : 'root-create', {
        ...
        path: form.get('path'),
        requiredMountPath: form.get('requiredMountPath'),
        kind: form.get('kind'),
